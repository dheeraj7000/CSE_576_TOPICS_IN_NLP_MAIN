...
OPEN_TAGS = cfg.get_special_tokens()[:-1]   # Opening connector tags
...
def stream_tokens_from_files(files):
    """Yields (token_id, fidx, sidx, pos) for all tokens in all files."""
...
def token_batch_streamer(files, batch_size, open_tag_ids, close_tag_id, boost, max_batches=None):
    ...
    while file_pointer < total_files and (max_batches is None or printed_batches < max_batches):
        ...
        if not chunk_files:
            break
        print(f"\n--- Processing file chunk ({file_pointer} to {file_pointer+len(chunk_files)-1}) ---")
        token_stream = stream_tokens_from_files(chunk_files)
        for tid, fidx, sidx, pos in token_stream:
...
if __name__ == "__main__":
    ...
    for i, (tokens, amps, sources) in enumerate(token_batch_streamer(train_files, BATCH_SIZE, open_tag_ids, close_tag_id, BOOST, max_batches=MAX_BATCHES_TO_PRINT)):
        print(f"\n================= BATCH {i+1} =================")
        print("Tokens:")
        print(tokens)
        print("Amps:")
